...
`Thead` accepts a `repeat` prop (default: `true`) to control whether headers repeat on each page. `Tr` accepts a `keep` prop to prevent a row from splitting across pages.
